...
- **创建与冻结**：`blind_rounds` 从同一项目选 2~3 个历史版本，创建时冻结各版本快照、对照分组与**最少有效提交人数**（`min_submitters`），此后项目继续编辑不影响本轮。分组用统一边模型：稳定句子编号相同为满分边，其余跨版本句两两算「0.65·文本相似（bigram Dice）+ 0.35·时间接近」内容边，确定性贪心并查集合并且**每组每版本至多一句**；一句在同一其他版本有 ≥2 个强候选判为**一对多**，其内容边全部排除——落单的新增/删除/一对多内容全部单列进 `unmatched`，绝不硬配成组。
...
- **记录**：创建/保存/提交/拒绝/揭示/关闭写 `blind_events`（页面「操作记录」可查）与审计表（`blind-create/save/submit/reject/reveal/close`）。
