...
/* 4d) 真正的前/后缀拆分（实测回归）：一版整句，另一版切成两个真正的半句——
       前半句沿用原稳定编号、后半句使用新编号。新编号的后半句只有半段文本
       与整句重叠（相似度约 0.5~0.6），若一对多判定另设更严的强候选门槛，
       就会漏判：前半句仍靠编号边与整句进对照项，后半句被误标成无对应。
       修正后：整句与两个半句都要单列为一对多，审阅列表不得出现这些候选。 */
r = buildComparison([
  V(0, [cue('c1', 0, 4000, '这是一条需要被拆成两个真正半句的完整字幕')]),
  V(1, [
    cue('c1', 0, 2000, '这是一条需要被拆成两个'),
    cue('n1', 2000, 4000, '真正半句的完整字幕'),
  ]),
]);
ok(r.items.length === 0, '真正前/后缀拆分不组成任何对照项（保留编号的前半句也不与整句配对）');
ok(r.unmatched.length === 3 && r.unmatched.every((u) => u.reason === REASON_ONE_TO_MANY),
  '整句、保留编号的前半句、新编号的后半句全部单列（一对多），后半句不被标成无对应');
ok(!r.items.some((i) => i.candidates.some((c) => c.cueId === 'c1' || c.cueId === 'n1')),
  '这次拆分涉及的整句与两个半句都不出现在审阅列表候选中');

/* 4e) 4d 的三版本形态：第三版仍是同编号整句。拆分分量必须沿对应边扩展到
       另一版本的整句——两个整句与两个半句共四句全部单列为一对多。 */
r = buildComparison([
  V(0, [cue('c1', 0, 4000, '这是一条需要被拆成两个真正半句的完整字幕')]),
  V(1, [
    cue('c1', 0, 2000, '这是一条需要被拆成两个'),
  ...
  V(2, [cue('c1', 0, 4000, '这是一条需要被拆成两个真正半句的完整字幕')]),
]);
ok(r.items.length === 0, '三版本下真正前/后缀拆分仍不组成任何对照项');
ok(r.unmatched.length === 4
  && r.unmatched.every((u) => u.reason === REASON_ONE_TO_MANY)
  && r.unmatched.filter((u) => u.cueId === 'c1').length === 3
  && r.unmatched.some((u) => u.cueId === 'n1'),
  '两个版本的整句与两个半句共四句全部单列（一对多）');
